    ...
    private fun processMessageContent(data: String) {
        ...
        context.getActivity()?.runOnUiThread {
            val chunkPath = if (chunk.data.isEnd()) {
                //TODO 缓存
                Log.w(TAG, "sse end, trace_id:$traceId")
            ...
            listener?.onReceiveChunk(MediaDataSource(chunk.trace_id, chunkPath, format, chunk.data.isEnd(), decodeHex(chunk.data.audio)))
        }
    }
...
data class MediaDataSource(
     ...
     */
    val isEnd: Boolean,
    val byteArrayAudio: ByteArray? = null
)
