        ...
        if (VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ...
            decorView.setOnApplyWindowInsetsListener(new View.OnApplyWindowInsetsListener() {
                ...
                public WindowInsets onApplyWindowInsets(View v, WindowInsets insets) {
                    ...
                    // 获取刘海参数
                    DisplayCutout displayCutout = decorView.getRootWindowInsets()
                            .getDisplayCutout();
                    if (displayCutout != null) {
                        // 如果有刘海
                        cutoutHeight = cutOutLeft = Math.max(displayCutout.getSafeInsetTop(),
                                displayCutout.getSafeInsetLeft());
                    }
                    decorView.setOnApplyWindowInsetsListener(null);

                    //listener.onApplyWindowInsets方法，会拦截View
                    // .onApplyWindowInsets方法，从而导致contentView拓展到navigator的下面，这是调用一下默认实现。
...
//            return;
//        }
//        if (VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) {
//            WindowManager.LayoutParams lp = window.getAttributes();
//            try {
...
//                        DisplayCutoutWrapper displayCutoutWrapper = windowInsetsWrapper
//                                .getDisplayCutoutWrapper();
//                        if (displayCutoutWrapper != null) {
//                            // 如果有刘海
//                            cutoutHeight = displayCutoutWrapper.getSafeInsetTop();
//                            if (displayCutoutWrapper.getBoundingRects() != null
//                                    && displayCutoutWrapper.getBoundingRects().size() >= 1) {
...
//    }
    ...
    private static boolean isLenovoBezelLessDevice(Context context) {
        try {
            return context.getResources().getBoolean(context.getResources()
                    .getIdentifier("config_screen_has_notch", "bool", "android"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
    ...
    private static boolean isSamsungBezelLessDevice() {
        try {
            ...
            final int resId = res
                    .getIdentifier("config_mainBuiltInDisplayCutout", "string", "android");
    ...
    public static int[] getMargins(Context context) {
        ...
        switch (deviceBrand) {
            ...
            case DEVICE_BRAND_VIVO_BEZEL_LESS: {
                margins[0] = margins[2] = (int) (YWDeviceUtil.getStatusBarHeight() * 1.5f);
                margins[1] = YWDeviceUtil.getStatusBarHeight();
                margins[3] = (int) (YWDeviceUtil.getStatusBarHeight() * 0.75f);
                break;
            }
            case DEVICE_BRAND_HUAWEI_BEZEL_LESS: {
                margins[0] = margins[1] = margins[2] = YWDeviceUtil.getStatusBarHeight();
                margins[3] = 0;
                break;
            }
            ...
            case DEVICE_BRAND_SAMSUNG_BEZEL_LESS: {
                ...
                margins[1] = cutoutHeight == 0 ? YWDeviceUtil.getStatusBarHeight() : cutoutHeight;
                margins[3] = 0;
                break;
            }
    ...
    public static boolean isFillCutout(Context context, int deviceBrand) {
        ...
        switch (deviceBrand) {
            case DEVICE_BRAND_OPPO_BEZEL_LESS: {
                ...
                try {
                    Class ColorDisplayCompatUtils = Class
                            .forName("com.color.util.ColorDisplayCompatUtils");
                    Method getInstance = ColorDisplayCompatUtils.getMethod("getInstance");
                    Object sColorDisplayCompatUtils = getInstance.invoke(null);
                    Method method = ColorDisplayCompatUtils
                            .getMethod("shouldNonImmersiveAdjustForPkg", pkg.getClass());
            ...
            case DEVICE_BRAND_HUAWEI_BEZEL_LESS: {
                // 华为刘海屏
                isEnableCutout = Settings.Secure
                        .getInt(context.getContentResolver(), DISPLAY_NOTCH_STATUS, 0) == 0;
                break;
            }
            case DEVICE_BRAND_XIAOMI_BEZEL_LESS: {
                // 小米刘海屏
    ...
    public static void addImmersiveStatusBarFlag(Activity activity) {
        ...
        if (!isSpecificRomVersion21()) {// 5.0以上使用FLAG_DRAWS_SYSTEM_BAR_BACKGROUND
...
//        setStatusBarLightMode(activity);
    ...
    public static boolean isSpecificRomVersion21() {
        if (!SPECIFIC_ROM_VERSION_HAS_CHECKED) {
            ...
            SPECIFIC_ROM_VERSION_NAME = YWDeviceUtil.getSystemProperty("ro.build.version.emui");
        }
    ...
    public static void setStatusBarWithColor(Activity activity, int color) {
        ...
        } else if (VERSION.SDK_INT >= 19) {// 4.4以上使用FLAG_TRANSLUCENT_STATUS
    ...
    private static void createStatusBar(Activity activity, int color) {
        ...
        FrameLayout.LayoutParams params = new FrameLayout.LayoutParams(
                ...
                YWDeviceUtil.getStatusBarHeight());
...
//                        SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
//            } else if (statusbar_type == 1000) {
//                setStatusBarWithColor(activity, Color.GRAY); //状态栏设为灰色
//            }
//        } else {
//
//            if (StatusBarBlackTextFixer.MIUISetStatusBarLightMode(activity, true)) {
//                statusbar_type = 1;
//            } else if (StatusBarBlackTextFixer
...
//    }
    ...
    public static void setScreenDisplay(Context context, Window window, boolean isShowStatusBar,
            boolean isShowNavigation) {
        setStatusBar(context, window, isShowStatusBar);
        setNavigation(context, window, isShowNavigation);
    }
     ...
     * @param isShowNavigation 是否显示虚拟键
     */
    public static void setScreenDisplay(Context context, Window window, boolean isShowStatusBar,
            boolean isLayoutStatusBar, boolean isShowNavigation, boolean isLayoutNavigation) {
        setStatusBar(context, window, isShowStatusBar, isLayoutStatusBar);
        setNavigation(context, window, isShowNavigation, isLayoutNavigation);
    }

    public static void setStatusBar(Context context, Window window, boolean isShowStatusBar) {
        setStatusBar(context, window, isShowStatusBar, true);
    }
     ...
     */
    public static void setStatusBar(Context context, Window window, boolean isShowStatusBar,
            boolean isLayoutStatusBar) {
        if (window == null || context == null) {
            return;
        }

        int curSystemUiVisibility = window.getDecorView().getSystemUiVisibility();
        int bezelLessWithBrand = SpecialScreenUtils.isBezelLessWithDeviceBrand(context);

        curSystemUiVisibility |= SYSTEM_UI_FLAG_IMMERSIVE_STICKY;
        if (isLayoutStatusBar) {
            // 设置全屏显示
            curSystemUiVisibility |= SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN;
        } else {
            curSystemUiVisibility &= ~SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN;
        }

        if (bezelLessWithBrand == 0) {
            // 如果不是刘海屏
            if (isShowStatusBar) {
                curSystemUiVisibility &= ~SYSTEM_UI_FLAG_FULLSCREEN;
            } else {
                curSystemUiVisibility |= SYSTEM_UI_FLAG_FULLSCREEN;
            }
        } else {
            // 如果是刘海屏
            if (SpecialScreenUtils.isFillCutout(context, bezelLessWithBrand)) {
                // 设置全屏显示
                curSystemUiVisibility |= SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN;
                // 如果填充凹形区
                if (!isShowStatusBar) {
        ...
        if ((curSystemUiVisibility & SYSTEM_UI_FLAG_FULLSCREEN) == SYSTEM_UI_FLAG_FULLSCREEN) {
            window.addFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
        } else {
            window.clearFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN);
        }
        window.getDecorView().setSystemUiVisibility(curSystemUiVisibility);
    }

    public static void setNavigation(Context context, Window window, boolean isShowNavigation) {
        setNavigation(context, window, isShowNavigation, false);
    }
    ...
    public static void setNavigation(Context context, Window window, boolean isShowNavigation,
        ...
        if (isLayoutNavigation) {
            curSystemUiVisibility |= SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION;
        } else {
            // 设置显示虚拟键
            curSystemUiVisibility &= ~SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION;
        }
        ...
        if (bezelLessWithBrand == 0) {
            ...
            if (isShowNavigation) {
                // 显示虚拟键
                curSystemUiVisibility &= ~SYSTEM_UI_FLAG_HIDE_NAVIGATION;
            } else {
                // 隐藏虚拟键
                curSystemUiVisibility |= SYSTEM_UI_FLAG_HIDE_NAVIGATION;
            }
    ...
    public static void addImmersiveStatusBarFlag(Window window) {
        //某些版本rom不适用5.0以上的设置方式
        if (!isSpecificRomVersion21()) {// 5.0以上使用FLAG_DRAWS_SYSTEM_BAR_BACKGROUND
            // FLAG_TRANSLUCENT_STATUS=0x04000000
            window.clearFlags(0x04000000);
            ...
            // FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS=0x80000000
            window.addFlags(0x80000000);
        } else {// 4.4以上使用FLAG_TRANSLUCENT_STATUS
            window.clearFlags(0x80000000);
            window.addFlags(0x04000000);
        }
        setStatusBarWithColor(window, Color.TRANSPARENT);
//        setStatusBarLightMode(window);
    }

    public static void setStatusBarWithColor(Window window, int color) {
        if (VERSION.SDK_INT >= 21) {// 5.0以上使用FLAG_DRAWS_SYSTEM_BAR_BACKGROUND
            try {
                Class<?> windowClass = Class.forName("android.view.Window");
                Method setSBCMethod = windowClass.getDeclaredMethod(
...
//        if (statusbar_type > 0) {
//            if (statusbar_type == 1) {
//                StatusBarBlackTextFixer.MIUISetStatusBarLightMode(window, true);
//            } else if (statusbar_type == 2) {
//                StatusBarBlackTextFixer.FlymeSetStatusBarLightMode(window, true);
//            } else if (statusbar_type == 3) {
//                window.getDecorView().setSystemUiVisibility(
//                        SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
//            } else if (statusbar_type == 1000) {
//                setStatusBarWithColor(window, window.getContext().getResources()
//                        .getColor(R.color.status_bar_bg)); //状态栏设为灰色
//            }
//        } else {
//            if (VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
//
//                if (StatusBarBlackTextFixer.MIUISetStatusBarLightMode(window, true)) {
//                    statusbar_type = 1;
//                } else if (StatusBarBlackTextFixer.FlymeSetStatusBarLightMode(window, true)) {
//                    statusbar_type = 2;
//                } else if (VERSION.SDK_INT >= Build.VERSION_CODES.M) {
//                    window.getDecorView().setSystemUiVisibility(
//                            SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN | SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
